<div id="edit" class="tab-pane">
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <form id="add-clients" action="{{!empty($user->id)? route('clients.update',['id' => $user->id]) :route('clients.store')}}" method="post" class="form-horizontal">
                {{ csrf_field() }}
                @if(!empty($user->id))
                    {{method_field('PUT')}}
                @endif
                <section class="panel">
                    <header class="panel-heading">
                        ...
                        @if(!empty($user->id))
                        <h2 class="panel-title">Редактирование клиента - {{$user->name}}</h2>
                        <p class="panel-subtitle">
                            Семь раз подумай, один раз обнови =)
                        </p>
                        @else
                            <h2 class="panel-title">Создание аккаунта клиента</h2>
                            <p class="panel-subtitle">
                    ...
                    <div class="panel-body">
                        ...
                        <select class="form-control mb-md" name="status">
                            <option {{($user->getClientInfo->status == 1) ? 'selected' :''}} value="1">Активирован</option>
                            <option {{($user->getClientInfo->status == 0) ? 'selected' :''}} value="0">Деактевирован</option>
                        </select>
                                </div>
                            </div>
