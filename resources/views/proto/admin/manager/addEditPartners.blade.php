<div id="edit" class="tab-pane">
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <form id="add-clients" action="{{!empty($user->id)? route('partners.update',['id' => $user->id]) :route('partners.store')}}" method="post" class="form-horizontal">
                {{ csrf_field() }}
                @if(!empty($user->id))
                    {{method_field('PUT')}}
                @endif
                <section class="panel">
                    <header class="panel-heading">
                        ...
                        @if(!empty($user->id))
                        <h2 class="panel-title">Редактирование партнера - {{$user->name}}</h2>
                        <p class="panel-subtitle">
                            Семь раз подумай, один раз обнови =)
                        </p>
                        @else
                            <h2 class="panel-title">Создание аккаунта партнера</h2>
                            <p class="panel-subtitle">
